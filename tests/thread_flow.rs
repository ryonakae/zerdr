...
#[test]
fn an_empty_workspace_gets_a_new_tab_and_a_default_pi_agent() {
    let fixture = Fixture::new();
    ...
    assert!(
        log.contains("herdr\t--session default agent start zed-1 --kind pi --pane w1:p9"),
        "{log}"
    );
    assert!(log.contains("agent attach w1:p9"), "{log}");
}
...
fn create_makes_the_workspace_binds_it_and_starts_an_agent() {
    ...
    assert!(
        log.contains("agent start zed-1 --kind pi --pane w7:p1"),
        "{log}"
    );
    assert!(log.contains("agent attach w7:p1"), "{log}");
...
/// Two threads racing on an empty workspace must each end up with their own agent and
/// never contend for one pane: serializing resolve-then-lease is what makes the second
/// invocation observe the first agent as taken instead of failing to lease it.
#[test]
fn concurrent_bare_threads_each_get_their_own_agent() {
    let fixture = Fixture::new();
    ...
    let log = fixture.env.read_log();
    assert_eq!(log.matches("agent start").count(), 2, "{log}");
    assert!(
        log.contains("agent start zed-1 --kind pi --pane w1:p1"),
        "{log}"
    );
    assert!(
        log.contains("agent start zed-2 --kind pi --pane w1:p2"),
        "{log}"
    );
    assert_eq!(log.matches("agent attach").count(), 2, "{log}");
}
