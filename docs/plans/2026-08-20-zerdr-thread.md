...
- **R16:** A Zed configuration file that is a symlink (commonly into a dotfiles checkout) is written through to its target, so the symlink survives and the dotfiles copy stays the source of truth. A symlink with no target is refused by name. Added 2026-08-21 after manual validation hit the previous blanket refusal; user decision: write through rather than skip the setting or keep failing. This replaces the old refusal for `tasks.json` too, so both Zed files behave the same.
...
- Task-level self-review cleanups before commit: a `Session` struct now carries herdr/leases/session name/socket (also keeping the argument count under clippy's limit); redundant Herdr calls were removed (`focus_workspace` and the sidebar label share one `workspace list`, and `generate_agent_name` reuses the `agent list` the resolver already loaded, so `agent list` is fetched once and `workspace list` at most twice — once under the resolve lock, once afterwards for fresh focus state); `create` is destructured from the subcommand instead of re-matched.

**Fixes from the independent review (no blocking or high findings):**
- The monitor now waits on a condvar instead of sleeping, so detaching returns the terminal to Zed immediately. Sleeping meant every close stalled for up to a full poll interval — 2 s at the default, invisible to tests because they all set a 20 ms interval. Covered by `detaching_does_not_wait_for_the_next_poll`, which uses a 30 s interval and asserts a prompt exit.
- A failed `workspace list` in `run` no longer collapses to an empty list. Doing so made the D6 guard read "not focused" and force a `workspace focus`, which is exactly the follow-mode focus-steal D6 exists to prevent. zerdr now warns and leaves focus alone. Covered by `an_unreadable_workspace_list_leaves_herdr_focus_untouched`.
- The resolve lock moved from one fixed path to `ThreadLeaseSet::resolve_lock_path(session, socket)`, so threads in unrelated Herdr sessions no longer serialize against each other as R9 requires. Covered by `the_resolve_lock_is_scoped_to_one_session_and_socket`.
...
- Task-level self-review additions: `settings.json` is now backed up under `<state>/backups/settings-*.jsonc` before either mutation, matching how the tasks file is treated (it was being rewritten with no backup — the more serious gap since this file is entirely the user's). `backup_before_mutation` takes a label. A regression test asserts the backup content.
- **Follow-up during manual validation (R16):** `zerdr setup` aborted with "refusing to replace symlinked Zed configuration file" because the tester's `settings.json` is a symlink into their dotfiles. The old blanket refusal existed because the atomic temp-plus-rename write would have replaced the link with a regular file and disconnected it from dotfiles; it had never fired before because only `tasks.json` went through that path and that file was not symlinked. `read_optional_file` no longer refuses symlinks, and `setup`/`uninstall` resolve each Zed file once with `resolve_config_file` and operate on the resolved path, so the link is preserved and the real file is updated. `doctor` needed no change (`read_to_string` already follows links). The failure was detected before any mutation, so the tester's environment was untouched.
...
| R16 | Task 4 | symlinked settings and tasks write-through test (links survive, targets updated, uninstall removes); broken-symlink refusal test |
