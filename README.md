...
- **You leave Zed (macOS).** A thread hosted in Zed's terminal watches the frontmost application; once Zed has been in the background for a second it detaches. Switching to a ghostty `herdr` on the same desk is enough. Working in Zed's editor never detaches, because Zed stays frontmost.
